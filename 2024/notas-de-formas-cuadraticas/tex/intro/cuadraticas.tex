\theoremstyle{plain}
\newtheorem{teoIntroCuad}{Teorema}[section]
\newtheorem{coroIntroCuad}[teoIntroCuad]{Corolario}
\newtheorem{lemaIntroCuad}[teoIntroCuad]{Lema}

\theoremstyle{definition}
\newtheorem{defIntroCuad}[teoIntroCuad]{Definici\'on}
\newtheorem{obsIntroCuad}[teoIntroCuad]{Observaci\'on}
\newtheorem{ejemIntroCuad}[teoIntroCuad]{Ejemplo}
\newtheorem{ejerIntroCuad}[teoIntroCuad]{Ejercicio}
...
\begin{enumerate}[(I)]
	\item\label{item:cuadraticas:ejemplos:i}
		$x^2+y^2+z^2$
	\item\label{item:cuadraticas:ejemplos:ii}
		$x^2+5xy-y^2$
	\item\label{item:cuadraticas:ejemplos:iii}
		$2x^2+3y^2$
	\item\label{item:cuadraticas:ejemplos:iv}
		$x_1^2+\,\cdots\,+x_n^2$
	\item\label{item:cuadraticas:ejemplos:v}
		$x_1^2+\,\cdots\,+x_p^2-x_{p+1}^2-\,\cdots\,-x_n^2$
\end{enumerate}
%
Los ejemplos anteriores, salvo \eqref{item:cuadraticas:ejemplos:ii}, son
ejemplos de formas cuadr\'aticas \emph{diagonales}, dado que no aparecen
t\'erminos cruzados. La forma \eqref{item:cuadraticas:ejemplos:ii} se puede
...
\end{displaymath}
%
Las formas \eqref{item:cuadraticas:ejemplos:iv} y
\eqref{item:cuadraticas:ejemplos:v} provienen de formas bilineales sim\'etricas
que conocemos:
\begin{displaymath}
	\begin{aligned}
...
\eqref{item:cuadraticas:ejemplos:iii} es diagonal y, sobre $\bb R$, es
...
\end{displaymath}
%
tiene soluci\'on en variable real, pero no tiene soluci\'on si $x$ e $y$ s\'olo
pueden tomar valores racionales.

Sobre un cuerpo de caracter\'{\i}stica distinta de $2$, toda forma cuadr\'atica
es diagonalizable. Esto deja de ser cierto sobre un cuerpo de
caracter\'{\i}stica $2$. Si, adem\'as, todo elemento no nulo del cuerpo de base
es un cuadrado --en el cuerpo--, entonces toda forma es equivalente a
\eqref{item:cuadraticas:ejemplos:iv}.
...
\subsection{Definiciones y ejemplos}%
	\label{subsec:intro:cuadraticas:definiciones}
Sea $F$ un cuerpo de caracter\'{\i}stica impar y sea $V$ un espacio vectorial
sobre $F$.

\begin{defIntroCuad}\label{def:cuadraticas:definicion}
	Una \emph{forma cuadr\'atica} en $V$ es una funci\'on
	$Q:\,V\rightarrow F$ que cumple:
	\begin{enumerate}[(i)]
		\item\label{item:cuadraticas:definicion:homogenea}
			$Q(c\,v)=c^2\,Q(v)$, para todo $v\in V$ y toda
			$c\in F$, y
		\item\label{item:cuadraticas:definicion:bilineal}
			la funci\'on
			\begin{math}
	...
	\eqref{item:cuadraticas:definicion:bilineal} se denomina \emph{forma %
	bilineal asociada} a la forma cuadr\'atica $Q$.
\end{defIntroCuad}

\begin{obsIntroCuad}\label{obs:cuadraticas:definicion}
	La relaci\'on entre la forma cuadr\'atica $Q$ y su forma bilineal
	asociada $B$ se puede expresar como:
	\begin{equation}
		\label{eq:cuadraticas:definicion:bilineal}
	...
	\begin{equation}
		\label{eq:cuadraticas:definicion:perpendicular}
...
\begin{obsIntroCuad}\label{obs:cuadraticas:definicion:polinomios}
	Inductivamente, de la identidad
	\eqref{eq:cuadraticas:definicion:bilineal}, se deduce que
	\begin{displaymath}
	...
	base y usando \eqref{item:cuadraticas:definicion:homogenea},
	\begin{equation}
		\label{eq:cuadraticas:definicion:polinomios}
	...
	\eqref{eq:cuadraticas:definicion:polinomios}, la funci\'on
	$Q:\,V\rightarrow F$ definida por
	\begin{math}
		Q(x_1\,e^1+\,\cdots\,+x_n\,e^n):=f(\lista x{n})
	\end{math} --una vez fijada una base-- verifica
	\eqref{item:cuadraticas:definicion:homogenea} y la funci\'on asociada
	$B=B_Q$ est\'a dada por
	\begin{equation}
		\label{eq:cuadraticas:definicion:polinomio:bilineal}
		B(v,w) % \,=\,\tfrac 1{2}\,\big(Q(v+w)\,-\,Q(v)\,-\,Q(w)\big)
...
\footnote{
	Teorema~\ref{teo:matrices:matrices}.
}
...
singulares, etc.%
\footnote{
	Teorema~\ref{teo:matrices:simetria}, Teorema~%
...
\begin{obsIntroCuad}\label{obs:cuadraticas:definicion:bilineal}
	Toda forma cuadr\'atica se puede recuperar de su forma bilineal
	asociada evaluando en la diagonal:
	\begin{equation}
		\label{eq:cuadraticas:definicion:bilineal:diagonal}
		Q(v)\,=\,B(v,v)
		\text{ .}
	\end{equation}
	%
	Rec\'{\i}procamente, dada una forma bilineal $B$, la funci\'on $Q$
	definida por \eqref{eq:cuadraticas:definicion:bilineal:diagonal} es una
	forma cuadr\'atica y, si $B$ es sim\'etrica, la forma bilineal asociada
	a $Q$ es $B_Q=B$, por polarizaci\'on.%
	\footnote{
		Teorema~\ref{teo:definiciones:polarizacion}.
	}
\end{obsIntroCuad}

La Observaci\'on~\ref{obs:cuadraticas:definicion:bilineal} muestra que existe
...
en t\'erminos de $Q$, por \eqref{eq:cuadraticas:definicion:perpendicular}.
...
\begin{ejemIntroCuad}\label{ejem:cuadraticas:traza}
...
\begin{ejemIntroCuad}\label{ejem:cuadraticas:binaria}
...
\begin{defIntroCuad}\label{def:cuadraticas:discriminante}
...
\begin{ejemIntroCuad}\label{ejem:cuadratiacs:hiperbolico}
	El discriminante de la forma $x^2-y^2$ en $F^2$ es $-1$.
\end{ejemIntroCuad}

\begin{defIntroCuad}\label{def:cuadraticas:espacio}
...
\begin{teoIntroCuad}\label{teo:cuadraticas:ortogonal:discriminante}
	Si $V=W\oplus U$ con $W\perp U$, entonces
	$\discriminante\,V=\discriminante\,W\,\discriminante\,U$.
\end{teoIntroCuad}

Para diagonalizar una forma cuadr\'atica, usamos los Lemas~%
...
\begin{ejerIntroCuad}\label{ejer:cuadraticas:isotropico}
...
\begin{defIntroCuad}\label{def:cuadraticas:equivalencia}
...
\begin{defIntroCuad}\label{def:cuadraticas:representa}
...
\begin{teoIntroCuad}\label{teo:cuadraticas:hiperbolico}
...
\begin{proof}
	...
	forma del Ejemplo~\ref{ejem:cuadraticas:equivalencia:hiperbolico}.

	Si $Q=x^2-y^2$, entonces $\discriminante\,Q=-1$.
\end{proof}

\subsection{Clasificaci\'on de formas cuadr\'aticas}%
	\label{subsec:intro:cuadraticas:clasificacion}
\begin{teoIntroCuad}\label{teo:cuadraticas:arquimedianas}
...
\begin{ejerIntroCuad}\label{ejer:cuadraticas:gauss}
...
\begin{teoIntroCuad}\label{teo:cuadraticas:finitos:representa}%
	\footnote{
		C.f. el Lema~\ref{lema:parias:finitos:representa}.
	}
...
\begin{lemaIntroCuad}\label{lema:cuadraticas:finitos:representa}
	Si $a,b,c\in\bb F^\times$, la ecuaci\'on
	\begin{equation}
		\label{eq:cuadraticas:finitos:representa}
		a\,x^2\,+\,b\,y^2\,+\,c\,=\,0
	\end{equation}
	%
...
\begin{proof}
	...
	la ecuaci\'on \eqref{eq:cuadraticas:finitos:representa} tiene
	soluci\'on.
\end{proof}

\begin{teoIntroCuad}\label{teo:cuadraticas:universal}%
	\footnote{
		C.f. el Lema~\ref{teo:parias:universal}.
	}
...
\begin{defIntroCuad}\label{def:cuadraticas:universal}
	Una forma cuadr\'atica $Q:\,V\rightarrow F$ tal que $Q(V)=F$ se dice
	que es \emph{universal}.
\end{defIntroCuad}

\begin{coroIntroCuad}\label{coro:cuadraticas:finitos:universal}%
	\footnote{
		C.f. el Lema~\ref{lema:parias:finitos:universal}.
	}
	Sea $\bb F$ un cuerpo finito de caracter\'{\i}stica impar y sea $(V,Q)$
	un espacio cuadr\'atico no degenerado sobre $\bb F$. Si
...
\begin{teoIntroCuad}\label{teo:cuadraticas:finitos}
...
\subsection{Un argumento geom\'etrico}%
	\label{subsec:intro:cuadraticas:geometria}
...
\begin{defIntroCuad}\label{def:cuadraticas:suma}
...
\begin{defIntroCuad}\label{def:cuadraticas:isotropico}
...
El Teorema~\ref{teo:cuadraticas:universal} implica que, en caracter\'{\i}stica
impar, toda forma isotr\'opica es universal.

\begin{defIntroCuad}\label{def:cuadraticas:hiperbolico}
...
degenerado e isotr\'opico (Teorema~\ref{teo:cuadraticas:hiperbolico}).

\begin{ejemIntroCuad}\label{ejem:cuadraticas:pseudoeuclideos}
...
\begin{teoIntroCuad}\label{teo:cuadraticas:isotropico}%
	\footnote{
		C.f. el Teorema~\ref{teo:parias:isotropico}.
	}
...
\begin{proof}
	...
	$B(u,v)\neq 0$ y $Q(u)=0$.%
	\footnote{
		C.f. la demostraci\'on del Teorema~%
	...
	subespacio no degenerado. Por el Teorema~%
...
\begin{equation}
	\label{eq:cuadraticas:isotropico:equivalente}
...
el discriminante:
\begin{equation}
	\label{eq:cuadraticas:isotropico:discriminante}
...
\begin{teoIntroCuad}\label{teo:cuadraticas:finitos:geometria}
...
\begin{lemaIntroCuad}\label{lema:cuadraticas:finitos:bajas}
...
\begin{proof}
	...
	Si $\dim\,V=2$, entonces $Q$ es universal, por el Corolario~%
	\ref{coro:cuadraticas:finitos:universal}. En particular,
...
?`C\'omo termina la demostraci\'on del Teorema~%
\ref{teo:cuadraticas:finitos:geometria}? Hay que probar que las formas que
...
\begin{teoIntroCuad}\label{teo:cuadraticas:nodegeneradas}
	Las siguientes afirmaciones son equivalentes:
	\begin{enumerate}[(i)]
		\item\label{item:cuadraticas:nodegeneradas:discriminante}
			la forma $Q$ es no degenerada, es decir,
			$\discriminante\,Q\neq 0$;
		\item\label{item:cuadraticas:nodegeneradas:variables}
			si $n=\dim\,V$, en ninguna base se puede expresar $Q$
			como un polinomio homog\'eneo de grado $2$ en menos de
			$n$ variables;
		\item\label{item:cuadraticas:nodegeneradas:derivadas}
...
\begin{proof}
	La afirmaci\'on \eqref{item:cuadraticas:nodegeneradas:discriminante} y
	la afirmaci\'on \eqref{item:cuadraticas:nodegeneradas:variables} son
	...
	\eqref{item:cuadraticas:nodegeneradas:discriminante} y la afirmaci\'on
	\eqref{item:cuadraticas:nodegeneradas:derivadas} son equivalentes: el
...
\begin{coroIntroCuad}\label{coro:cuadraticas:cuadricas}
...
% \subsection{Algunos invariantes}\label{subsec:intro:cuadraticas:invariantes}
...
\begin{ejerIntroCuad}\label{ejer:cuadraticas:norma}
...
\begin{ejerIntroCuad}\label{ejer:cuad:traza}
