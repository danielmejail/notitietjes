\theoremstyle{plain}
\newtheorem{teoIntroPar}{Teorema}[section]
\newtheorem{lemaIntroPar}[teoIntroPar]{Lema}

\theoremstyle{definition}
\newtheorem{defIntroPar}[teoIntroPar]{Definici\'on}
\newtheorem{obsIntroPar}[teoIntroPar]{Observaci\'on}
\newtheorem{ejemIntroPar}[teoIntroPar]{Ejemplo}
\newtheorem{ejerIntroPar}[teoIntroPar]{Ejercicio}

%-------------

% La definici\'on de forma cuadr\'atica sobre cuerpos de caracter\'{\i}stica $2$
% es similar a la definici\'on en caracter\'{\i}stica impar.
Sea $F$ un cuerpo de caracter\'{\i}stica $2$ y sea $V$ un espacio vectorial
sobre $F$.

\begin{defIntroPar}\label{def:parias:definicion}
...
\begin{obsIntroPar}\label{obs:parias:definicion}
...
\begin{obsIntroPar}\label{obs:parias:definicion:polinomios}
...
\begin{obsIntroPar}\label{obs:parias:coeficientes}
...
\begin{obsIntroPar}\label{obs:parias:matriz}
...
\begin{ejemIntroPar}\label{ejem:parias:binaria}
...
\begin{ejemIntroPar}\label{ejem:parias:falla}
...
Como se ve en el Ejemplo~\ref{ejem:parias:falla}, la aplicaci\'on
...
\subsection{Un sustituto de la diagonalizaci\'on}
	\label{subsec:intro:parias:simplecticas}
...
Corolario~\ref{coro:simplecticas:dimension}, $\dim\,V=2m$ y existe una base
...
\subsection{Formas cuadr\'aticas no degeneradas}%
	\label{subsec:intro:parias:nodegeneradas}
El Teorema~\ref{teo:cuadraticas:nodegeneradas} da condiciones equivalentes a la
...
\begin{teoIntroPar}\label{teo:parias:nodegeneradas}
...
\begin{defIntroPar}\label{def:parias:nodegeneradas}
	La forma cuadr\'atica $Q$ se dice \emph{no degenerada}, si verifica las
	condiciones del Teorema~\ref{teo:parias:nodegeneradas}. Un
...
\subsection{Ejemplos}\label{subsec:intro:parias:ejemplos}

\begin{ejemIntroPar}\label{ejem:parias:bajas}
...
\begin{ejemIntroPar}\label{ejem:parias:tres}
...
\begin{ejemIntroPar}\label{ejem:parias:extensiones}
...
\begin{ejemIntroPar}\label{ejem:parias:extensiones:bis}
...
\begin{obsIntroPar}\label{obs:parias:extensiones}
	Si $F$ es un cuerpo de caracter\'{\i}stica impar, una forma
	$Q:\,V\rightarrow F$ es no degenerada, si y s\'olo si
	(Teorema~\ref{teo:cuadraticas:nodegeneradas}) su discriminante es
	...
	\begin{equation}
		\label{eq:cuadraticas:extensiones}
...
\subsection{Clasificaci\'on en caracter\'{\i}stica $2$}%
	\label{subsec:intro:parias:clasificacion}

\begin{teoIntroPar}\label{teo:parias:universal}%
	\footnote{
		C.f. el Teorema~\ref{teo:cuadraticas:universal}.
	}
...
\begin{proof}
	...
	$B(v,w)\neq 0$. El argumento es, ahora, id\'entico al del Teorema~%
	\ref{teo:cuadraticas:universal}, con la diferencia del factor $2$.
\end{proof}

\begin{teoIntroPar}\label{teo:parias:isotropico}%
	\footnote{
		C.f. el Teorema~\ref{teo:cuadraticas:isotropico}.
	}
...
\begin{proof}
	Como en la demostraci\'on del Teorema~\ref{teo:parias:universal},
...
\begin{lemaIntroPar}\label{lema:parias:finitos:representa}%
	\footnote{
		C.f. el Teorema~\ref{teo:cuadraticas:finitos:representa}.
	}
...
\begin{lemaIntroPar}\label{lema:parias:finitos:universal}%
	\footnote{
		C.f. el Corolario~\ref{coro:cuadraticas:finitos:universal}.
	}
	Sea $\bb F$ un cuerpo finito de caracter\'{\i}stica $2$ y sea $(V,Q)$
	un espacio cuadr\'atico.%
	\footnote{
...
\begin{lemaIntroPar}\label{lema:parias:finitos:radical}
...
\begin{obsIntroPar}\label{obs:parias:nodegeneradas:par}
	Una consecuencia del Lema~\ref{lema:parias:finitos:radical} es que, en
	caracter\'{\i}stica $2$, cuando $\dim\,V$ es par, $Q$ es no degenerada,
	si y s\'olo si $B$ es no degenerada ($V^\perp=0$).
\end{obsIntroPar}

\begin{obsIntroPar}\label{obs:parias:perfectos}
...
\begin{defIntroPar}\label{def:parias:valoresespeciales}
...
\begin{obsIntroPar}\label{obs:parias:valoresespeciales}
...
El siguiente resultado juega el rol del Lema~%
\ref{lema:cuadraticas:finitos:bajas}.

\begin{teoIntroPar}\label{teo:parias:finitos:bajas}
...
\begin{proof}
	...
	$2$ es equivalente a una de estas formas, por el Lema~%
	\ref{lema:parias:finitos:universal}, $Q(v)=1$ para cierto $v\in V$.%
	\footnote{
		C.f. el argumento del Teorema~\ref{teo:cuadraticas:finitos}.
	}
	Por el Lema~\ref{lema:parias:finitos:radical}, $V^\perp=0$ y $B=B_Q$ es
	...
	Si $\dim\,V=3$, por el Lema~\ref{lema:parias:finitos:representa},
	existe $e\neq 0$ tal que $Q(e)=0$ y, por el Teorema~%
...
\begin{obsIntroPar}\label{obs:parias:finitos:bajas}
	Si $U=\generado{e,f}$ es un plano como en el Teorema~%
	\ref{teo:parias:isotropico} con $F=\bb F$ finito, entonces, aplicando
	el Teorema~\ref{teo:parias:finitos:bajas}, dado que $U$ contiene
	vectores isotr\'opicos, la forma $Q|_U$ debe ser equivalente a $x\,y$.
\end{obsIntroPar}

\begin{teoIntroPar}\label{teo:parias:finitos:geometria}
...
\begin{proof}
	Podemos asumir $n=\dim\,V\geq 4$. Entonces, por el Teorema~%
	\ref{lema:parias:finitos:representa}, existe $v\neq 0$ tal que
	$Q(v)=0$. Por el Teorema~\ref{teo:parias:isotropico}, existe $w$ tal
	que $B(v,w)=1$, $Q(w)=0$ y $U:=\generado{v,w}$ es no degenerado. Por
	la Observaci\'on~\ref{obs:parias:finitos:bajas}, $Q|_U$ es equivalente
	a $x\,y$.

	Si $n$ es par, por el Lema~\ref{lema:parias:finitos:radical},
	$V^\perp=0$ y $B$ es no degenerada. Entonces, por el Teorema~%
	...
	aplicando el Teorema~\ref{teo:parias:finitos:bajas}, est\'a
	...
	$W\cap V^\perp=0$). Por la Observaci\'on~%
	...
	\end{displaymath}
	%
	en $\bb F^3$ es no degenerada y, por el Teorema~%
	\ref{teo:parias:finitos:bajas}, equivalente a $x\,y+z^2$.
\end{proof}

El enunciado del Teorema~\ref{teo:parias:finitos:geometria} no garantiza que
...
caracter\'{\i}stica impar, Teorema~\ref{teo:cuadraticas:finitos:geometria},
...
\begin{defIntroPar}\label{def:parias:ceros}
...
\begin{ejemIntroPar}\label{ejem:parias:ceros}
...
\begin{obsIntroPar}\label{obs:parias:ceros}
...
\begin{ejerIntroPar}\label{ejer:parias:ceros}
	Calcular $|Q^{-1}(a)|$ para cada $a\neq 0$ para la forma $x\,y$.%
	\hint{
		Probar que $|Q^{-1}(1)|\geq 1$ y apelar a la Observaci\'on~%
		\ref{obs:parias:ceros} y al Ejemplo~\ref{ejem:parias:ceros}
	}
	Hacer lo mismo para la forma $x^2+x\,y+d\,y^2$.
\end{ejerIntroPar}

\begin{lemaIntroPar}\label{lema:parias:finitos:suma}
...
\begin{proof}
	...
	\end{displaymath}
	%
	Separar en casos $h(u)=0$ y $h(u)\neq 0$ y usar la Observaci\'on~%
	\ref{obs:parias:ceros} y el Ejemplo~\ref{ejem:parias:ceros}.
\end{proof}

\begin{teoIntroPar}\label{teo:parias:finitos:ceros}
	...
	\end{displaymath}
	%
	En particular, las dos formas de dimensiones pares del Teorema~%
...
\begin{defIntroPar}\label{def:parias:arf}
...
\begin{ejerIntroPar}\label{ejer:parias:arf}
...
\begin{ejerIntroPar}\label{ejer:parias:norma}
...
\begin{ejerIntroPar}\label{ejer:parias:binarias}
...
\begin{ejerIntroPar}\label{ejer:parias:democracia}
